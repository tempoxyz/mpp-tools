...
fn handle_generate_challenge_id(input: &str) {
    match serde_json::from_str::<serde_json::Value>(input) {
        Ok(params) => {
            ...
            let id = match generate_conformance_challenge_id(ChallengeIdInput {
                secret_key,
                realm: &realm,
                method: &method,
                intent: &intent,
...
fn generate_conformance_challenge_id(
    input: ChallengeIdInput<'_>,
) -> Result<String, std::fmt::Error> {
    type HmacSha256 = Hmac<Sha256>;

    let request_json = stable_json(input.request)?;
    ...
    let hmac_input = [
        ...
        input.intent,
        &request_b64,
        input.expires.unwrap_or(""),
        input.digest.unwrap_or(""),
        input.opaque.unwrap_or(""),
    ]
    .join("|");

    let mut mac = HmacSha256::new_from_slice(input.secret_key.as_bytes())
